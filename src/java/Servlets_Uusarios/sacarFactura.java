...
import ConectaBD.ExportrExcelFACTURA;
 ...
 * @author Alumno
 */
@WebServlet(name = "sacarFactura", urlPatterns = {"/sacarFactura"})
public class sacarFactura extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql="SELECT dp.cod_ped, dp.cod_art, dp.fecha_ped, (dp.cantidad*dp.precio_arts) AS total, p.cod_cli, c.nombre, c.apellidos FROM detallespedido dp RIGHT JOIN pedidos p ON dp.cod_ped=p.cod_ped RIGHT JOIN clientes c ON p.cod_cli=c.cod_cli RIGHT JOIN usuarios u ON c.cod_user=u.cod_user WHERE dp.cod_ped IS NOT NULL AND u.login='"+nombreusuario+"'";
            ...
            request.getSession().setAttribute("tuFACTURA", listaventas);
            ExportrExcelFACTURA exportar = new ExportrExcelFACTURA();
            exportar.export(nombreusuario);
            response.sendRedirect("mostrarfactura.jsp");
        } catch (Exception e) {
            e.printStackTrace();
        }
